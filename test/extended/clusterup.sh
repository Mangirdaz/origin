...
function os::test::extended::clusterup::registryconfig () {
    ...
    os::cmd::try_until_text "oc get pods -n default" "docker-registry-1-deploy"
    os::cmd::expect_success "oc login -u developer -p developer"
    token="$( oc whoami -t )"
    os::cmd::try_until_text "docker login docker-registry-default.127.0.0.1.nip.io -u developer -p ${token}" "Login Succeeded"
    ...
    os::cmd::expect_success "oc login -u developer -p developer"
    token="$( oc whoami -t )"
    os::cmd::try_until_text "docker login docker-registry-default.${ipaddr}.nip.io -u developer -p ${token}" "Login Succeeded"
    docker pull docker.io/busybox
...
function os::test::extended::clusterup::registryconfig_cleanup () {
    ...
    rm -rf /etc/docker*/certs.d/docker-registry-default*
}
